...
export const getHistoricoHDR = async id => {
  try {
    const response = await axiosService.get(`${ELTA_URL}/userflota/hdr_admin/general/${id}`)
...
export const postCargaHDR = async newCargaHDR => {
  try {
    ...
    console.log('No se pudo crear la Carga', error)
  }
}
