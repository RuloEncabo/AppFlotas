...
import { Grid, Switch, Box, Typography, InputLabel, FormHelperText, FormLabel, RadioGroup, FormControlLabel, Radio } from '@mui/material'
...
const FormAgregarNovedadFlota = ({ isOpen, onClose, dataUpdate, isAgregando, hdr_id ,setNovedades}) => {
  ...
  const [cargaNovedad, setCargaNovedad] = useState({
    ...
    nov_asignacion: dataUpdate?.nov_asignacion || "SIN ASIGNAR",
  })
      ...
      if (isAgregando) {
        ...
        for (const img of listaImagenes) {
          ...
          try {
            ...
            nuevaCargaNovedad = { ...nuevaCargaNovedad, nov_asignacion: "SIN ASIGNAR" };
        ...
        //await putNovedadHDR(nov_id, cargaNovedad)
                ...
                render={({ field: { onChange, value } }) => (
                  <DatePickerWrapper>
                    <DatePicker
                      ...
                      timeFormat='HH:mm'
                      timeIntervals={15}
                      selected={value ? new Date(cargaNovedad.nov_fecha) : null}
                      id='fecha-novedad'
...
export default FormAgregarNovedadFlota
